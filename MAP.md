...
| **On/off élément (1 2 3 4)** | 🟢 | — | **CÂBLÉ (A)** : boutons « Operateurs On-Off » (Voice.h) = on/off des 4 ÉLÉMENTS via l'**OUTPUT SELECT** (modèle réel SY77, identifié HW). OFF -> OUTSEL=0 (group 0x03 / param 0x08) ; ON -> restaure le routage des bascules G1/G2 (`sendOutSel`). Le routage voulu reste dans ELEMENT<n>GROUP1/2, rien à sauvegarder. `Element::setElementMuted`/`sendOutSel` (mute-aware). Boutons hors mode courant grisés (source `nombreElements`). Envoi au CLIC seulement. (Avant : MUTE via ELVL=0 — abandonné.) |
